...
use Zeero\SchemaBuilder\traits\dateType;
use Zeero\SchemaBuilder\traits\floatType;
use Zeero\SchemaBuilder\traits\modifier;
use Zeero\SchemaBuilder\traits\setType;
use Zeero\SchemaBuilder\traits\util;
...
class Table
{
    ...
    private function create_str(array $info)
    {
        ...
        // make sure that all fields wiil be not null
        $info['mod']['not null'] = '';
    ...
    // primary
    public function primary(string $name = 'ID')
    {
        $this->pk = $name;
        return $this;
    }
    ...
    public function results(bool $onlyFields = false)
    {
        ...
        if (isset($this->pk)) {
            $s = "`{$this->pk}` INT NOT NULL AUTO_INCREMENT +";
            $str = $s . $str;
        }
